...
export default function DashboardScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return items;
  }, [source, tab, typeFilter, groupFilter, urgencyFilter]);
  ...
  const daysUntilEligible = userData?.lastDonationDate?.toDate
    ? Math.max(0, 90 - Math.floor((Date.now() - userData.lastDonationDate.toDate().getTime()) / (1000 * 60 * 60 * 24)))
    : '—';
  const group = userData?.bloodGroup || '—';

  return (
    <SafeAreaView style={styles.root}>
      {/* Header area reflecting the SVG layout (solid theme version for performance) */}
      <View style={styles.headerArea}>
        ...
        {/* Search pill (visual only, keeps existing search/filter logic minimal here) */}
        ...
        <View style={styles.twinsRow}>
          <TouchableOpacity
            ...
            style={[styles.ctaCard, { backgroundColor: UI.colors.accent }]}
          >
          ...
          <TouchableOpacity
            ...
            style={[styles.ctaCard, { backgroundColor: UI.colors.blue }]}
          >
      ...
      {/* Filters bottom sheet (themed; trigger from your existing controls as needed) */}
...
function Card({ post, onPress }) {
  ...
  const boxBg = post.type === 'receiver' ? '#FFE4E6' : post.type === 'donor' ? '#ECFDF5' : UI.colors.surface;
  const boxTxt = post.type === 'receiver' ? UI.colors.accent : post.type === 'donor' ? '#047857' : UI.colors.text;

  return (
    <TouchableOpacity style={styles.card} activeOpacity={0.95} onPress={onPress}>
      <View style={[styles.leftBox, { backgroundColor: boxBg }]}>
        <Text style={[styles.leftBoxText, { color: boxTxt }]}>{(post.bloodGroup || '').toUpperCase()}</Text>
      </View>
...
function Pill({ label, selected, onPress }) {
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.9} style={[styles.pill, selected && styles.pillActive]}>
...
const styles = StyleSheet.create({
  ...
  headerBar: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 },
  ...
  searchPill: {
    height: 46, borderRadius: 23, backgroundColor: UI.colors.surface, paddingHorizontal: 16,
    flexDirection: 'row', alignItems: 'center', marginTop: 14, borderWidth: 1, borderColor: UI.colors.border, ...UI.shadow.sm,
  },
  ...
  ctaCard: { flex: 1, borderRadius: UI.radii.md, padding: 14, overflow: 'hidden', ...UI.shadow.md },
  ctaTitleWhite: { color: '#FFFFFF', fontWeight: '800', fontSize: 16 },
  ctaSubWhite: { color: '#FFFFFF', opacity: 0.95, marginTop: 2, fontSize: 12 },
  ctaChipWhite: { marginTop: 8, backgroundColor: '#FFFFFF', height: 28, borderRadius: 8, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 12 },
  ctaChipTextDark: { color: UI.colors.text, fontWeight: '700', fontSize: 12 },
  ctaChipGhost: { marginTop: 8, backgroundColor: '#FFFFFF22', borderColor: '#FFFFFF55', borderWidth: 1, height: 28, borderRadius: 8, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 12 },
  ctaChipTextWhite: { color: '#FFFFFF', fontWeight: '700', fontSize: 12 },

  kpiRow: { flexDirection: 'row', gap: 14, marginTop: 16 },
  kpiTall: { flex: 1, borderRadius: UI.radii.md, padding: 14, backgroundColor: UI.colors.surface, borderWidth: 1, borderColor: UI.colors.border, ...UI.shadow.sm },
  kpiStack: { flex: 1, gap: 8 },
  kpiSmall: { borderRadius: UI.radii.md, padding: 14, backgroundColor: UI.colors.surface, borderWidth: 1, borderColor: UI.colors.border, ...UI.shadow.sm },
  ...
  segment: { flexDirection: 'row', backgroundColor: UI.colors.surface, borderRadius: 17, padding: 4, gap: 6, ...UI.shadow.sm, borderWidth: 1, borderColor: UI.colors.border },
  ...
  card: {
    flexDirection: 'row', backgroundColor: UI.colors.surface, borderRadius: UI.radii.md,
    padding: 12, marginBottom: 12, borderWidth: 1, borderColor: UI.colors.border, ...UI.shadow.sm,
  },
  leftBox: { width: 56, height: 56, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginTop: 10, marginRight: 12 },
  ...
  primaryBtn: { paddingHorizontal: 16, height: 36, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  primaryBtnText: { color: '#FFFFFF', fontWeight: '800' },
  secondaryBtn: { backgroundColor: UI.colors.text, paddingHorizontal: 14, height: 36, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  secondaryBtnText: { color: '#FFFFFF', fontWeight: '800' },

  // Bottom sheet
  sheetBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.25)' },
  sheet: { position: 'absolute', left: 0, right: 0, bottom: 0, backgroundColor: UI.colors.surface, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 16, gap: 10 },
  ...
  pill: { height: 34, paddingHorizontal: 14, borderRadius: 18, backgroundColor: '#F2F4F8', justifyContent: 'center', alignItems: 'center' },
  ...
  // FAB
  fab: { position: 'absolute', right: 16, bottom: 24, height: 44, borderRadius: 22, paddingHorizontal: 16, backgroundColor: UI.colors.accent, flexDirection: 'row', alignItems: 'center', gap: 6, ...UI.shadow.lg },
